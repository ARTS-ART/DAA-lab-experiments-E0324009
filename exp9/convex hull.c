...
int compare(const void *p1, const void *q1)
{
    ...
    long long cross_product = (long long)p_diff.second * q_diff.first - (long long)q_diff.second * p_diff.first;
...
Pair *divide(Pair *a, int n, int *ret_size)
{
    ...
    int m = n / 2;
    Pair *left_points = (Pair *)malloc(m * sizeof(Pair));
    Pair *right_points = (Pair *)malloc((n - m) * sizeof(Pair));

    for(int i = 0; i < m; i++) left_points[i] = a[i];
    for(int i = 0; i < n - m; i++) right_points[i] = a[m+i];


    int left_size, right_size;

    Pair *left_hull = divide(left_points, m, &left_size);
    Pair *right_hull = divide(right_points, n - m, &right_size);
